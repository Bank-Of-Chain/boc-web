...
const useRedeemFeeBps = (props) => {
  ...
  const reload = () => {
    ...
    vaultContract.redeemFeeBps().then(setValue).catch(setError).finally(() => setLoading(false))
  }
  ...
  const set = (nextValue) => {
    ...
    vaultContract.connect(signer).setRedeemFeeBps(nextValue).then(tx => tx.wait()).then(reload).catch(error => {
